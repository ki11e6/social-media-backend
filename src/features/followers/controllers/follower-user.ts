...
export class Add {
  public async follower(req: Request, res: Response): Promise<void> {
    const { followerId } = req.params; //userId of follower.
    // update count in cache
    //update the user thats following current user.
    const followersCount: Promise<void> = followerCache.updateFollowersCountInCache(`${followerId}`, 'followersCount', 1);
    //update the count of followers in current user.
    ...
    //create a custom mongodb objectId
    const followerObjectId: ObjectId = new ObjectId();
    //followee data to sent back to client.
    ...
    //saving the follower and followee to cache.
